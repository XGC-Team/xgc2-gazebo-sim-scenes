...
void DynamicObstacle::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    ...
    for (int i = 0; i < this->pathWithAngle.size() - 1; ++i) {
        ...
        bool rotation =
            ((poseCurr[0] == poseNext[0]) and (poseCurr[1] == poseNext[1]) and (poseCurr[2] == poseNext[2]));
    ...
    if (this->pathWithAngle.size() != this->timeKnot.size() ||
        !xgc2_gazebo_scene::UniqueKeyframeIndices(this->timeKnot, &keyframe_indices) ||
        keyframe_indices.size() < 2) {
...
void DynamicObstacle::linear(const double& ts, const std::vector<std::vector<double>>& path,
                             const std::vector<double>& time, std::vector<std::vector<double>>& traj,
                             std::vector<double>& t) {
    traj.clear();
    t.clear();
...
void DynamicObstacle::sine(const double& ts, const std::vector<std::vector<double>>& path,
                           const std::vector<double>& time, std::vector<std::vector<double>>& traj,
                           std::vector<double>& t) {
    traj.clear();
    t.clear();
    ...
    for (int i = 0; i < int(path.size() - 1); i++) {
        ...
        double timeStep = (time[i + 1] - time[i]);
        for (int j = 0; j < int(timeStep / ts); j++) {
            // std::vector<double> p{0.5*sin((j*ts)*(2*M_PI)/(currP[0]-nextP[0])),
            // 0.5*sin((j*ts)*(2*M_PI)/(currP[1]-nextP[1])), 0, 0};
...
void DynamicObstacle::lls(const double& ts, const std::vector<std::vector<double>>& path,
                          const std::vector<double>& time, std::vector<std::vector<double>>& traj,
                          std::vector<double>& t, const int& order) {
    double dt = ts;
    ...
    for (int j = 0; j < predx.rows(); j++) {
        std::vector<double> p;
        if (j + 1 < predx.rows()) {
            p = std::vector<double>{predx(j, 0), predy(j, 0), path[0][2],
                                    atan2(predy(j + 1, 0) - predy(j, 0), predx(j + 1, 0) - predx(j, 0))};
        } else {
...
Eigen::MatrixXd DynamicObstacle::fit(const Eigen::MatrixXd& x, const Eigen::MatrixXd& y, const Eigen::MatrixXd& px,
                                     const int& order) {
    int numIter = 10;
