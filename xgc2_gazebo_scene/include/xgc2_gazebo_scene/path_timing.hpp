...
inline bool SegmentDuration(double distance, double speed, double* duration) {
    if (duration == nullptr || !std::isfinite(distance) || distance < 0.0 ||
        !std::isfinite(speed) || speed <= 0.0) return false;
    const double value = distance / speed;
    if (!std::isfinite(value) || (distance > 0.0 && value <= 0.0)) return false;
    *duration = value;
    return true;
}
...
inline bool UniqueKeyframeIndices(const std::vector<double>& times,
    ...
    for (std::size_t i = 0; i < times.size(); ++i) {
        ...
        if (i + 1 == times.size() || times[i + 1] != times[i]) indices->push_back(i);
    }
