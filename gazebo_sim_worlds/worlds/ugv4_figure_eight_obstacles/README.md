# UGV4 Figure-Eight Obstacles

通用静态 Gazebo Classic 场景，逐项复现 `paper-leader` 的
`ugv4_circle.launch` 障碍物配置。源实验的领导者轨迹实际为八字：

- 10 个障碍物；
- 位置、姿态和尺度沿用源配置；
- cube、sphere、cylinder 使用 Gazebo 原生 collision；
- 两个 `ugv_hexagon_1` 使用闭合凸三角网格作为真实静态物理 collision，
...
```text
world=/opt/ros/noetic/share/gazebo_sim_worlds/worlds/catalog/ugv4_figure_eight_obstacles.world
```
...
roslaunch gazebo_ros empty_world.launch \
  world_name:="$(rospack find gazebo_sim_worlds)/worlds/catalog/ugv4_figure_eight_obstacles.world" \
